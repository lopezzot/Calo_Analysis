...
def towercalibration():
	...
	DeltaTheta = math.pi/4/40
	thetas = [(math.pi/8/40)+DeltaTheta/6.*step for step in range(0,18)]
	thetas = [90.+x*180./math.pi for x in thetas]
	thetas = array('d',thetas)
	inputfiles = ["/home/software/Calo/results/newresults/ScanInsideTower/Step_"+str(i)+".root" for i in range(0,18)]
	...
	for counter, inputfile in enumerate(inputfiles):
		...
		ResponseScinHist = TH1F("responsescin_", str(counter+1)+"_scin", 500, 0., 1000.)
		ResponseCherHist = TH1F("responsecher_", str(counter+1)+"_cher", 300, 0., 600.)
	...
	ResponseRMSGraphScin = TGraph(n, thetas, ResponseRMSScin)
	ResponseRMSGraphScin.SetName("ResponseRMSGraphScin")
	ResponseRMSGraphCher = TGraph(n, thetas, ResponseRMSCher)
	ResponseRMSGraphCher.SetName("ResponseRMSGraphCher")
	ResponseMeanGraphScin = TGraphErrors(n, thetas, ResponseMeanScin, Zeros, Responseerrorsscin)
	print "Response scin: "+str(ResponseMeanScin)
	ResponseMeanGraphScin.SetName("ResponseMeanGraphScin")
	ResponseMeanGraphCher = TGraphErrors(n, thetas, ResponseMeanCher, Zeros, Responseerrorscher)
